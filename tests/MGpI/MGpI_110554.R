...
gsm <- getGEO("GSE110554")
data <- exprs(gsm[[1]])
data <- data[1:30000,c(4,9,29,33,40,44,8,14,16,32,46,48,11,12,18,23,25,31,13,17,20,24,30,42,1,3,10,19,38,47,2,5,7,41,43,49)]
## 4,9,29,33,40,44: Bcell
## 8,14,16,32,46,48: CD4T
## 11,12,18,23,25,31: CD8T
## 13,17,20,24,30,42: Mono
## 1,3,10,19,38,47: Neu
## 2,5,7,41,43,49: NK


test_function<-function(data,rate,Total_missing,missing_rate_threshold=0.6,
                        MAR_option = 'All',pram = 12,sd_scaler=4,nRep){
  
  # pram: parameter for SVD, PPCA, NIPALS, KNN
  # rate: MAR rate
  ################# Read-in Data #################
  data_grnd_full <- as.matrix(data)
  ...
  #ABDS_imputed_masked_out<-ABDS_imputed[mg_grnd_masked_out,]

  rmse_ABDS_all <- rmse(data_grnd_full,
                        ABDS_imputed)
  
  # nrmse_ABDS_sg <- nrmse(before_missing_masked_out,ABDS_imputed[mg_grnd_masked_out,])
  # nrmse_ABDS_non_sg <- nrmse(data_grnd_full[non_SG_masked_out,],ABDS_imputed[non_SG_masked_out,])
  ...
  # nrmse_swKNN_sg <- nrmse(before_missing_masked_out,swKNN_imputed[mg_grnd_masked_out,])
  # nrmse_swKNN_non_sg <- nrmse(data_grnd_full[non_SG_masked_out,],swKNN_imputed[non_SG_masked_out,])
  ...
  # nrmse_NIPALS_sg <- nrmse(before_missing_masked_out,NIPALS_imputed[mg_grnd_masked_out,])
  # nrmse_NIPALS_non_sg <- nrmse(data_grnd_full[non_SG_masked_out,],NIPALS_imputed[non_SG_masked_out,])
  ...
  # nrmse_SVD_sg <- nrmse(before_missing_masked_out,SVD_imputed[mg_grnd_masked_out,])
  # nrmse_SVD_non_sg <- nrmse(data_grnd_full[non_SG_masked_out,],SVD_imputed[non_SG_masked_out,])
  ...
  # nrmse_SVT_sg <- nrmse(before_missing_masked_out,SVT_imputed[mg_grnd_masked_out,])
  # nrmse_SVT_non_sg <- nrmse(data_grnd_full[non_SG_masked_out,],SVT_imputed[non_SG_masked_out,])
  ...
  # nrmse_PPCA_sg <- nrmse(before_missing_masked_out,PPCA_imputed[mg_grnd_masked_out,])
  # nrmse_PPCA_non_sg <- nrmse(data_grnd_full[non_SG_masked_out,],PPCA_imputed[non_SG_masked_out,])
  ...
  SGlist<-cotMG(Sest=before_missing_super_sample,cos.thres = 0.6,per=50)
...
test1<-test_function(data = data ,rate = 0.4,Total_missing = 1080000*0.4,pram = 10,sd_scaler=4,nRep=c(6,6,6,6,6,6))

result_110554<-NULL
rate_range<-c(0.2,0.4,0.6,0.8)
Total_missing_range<-c(1080000*0.2,1080000*0.3,1080000*0.4)
for (i in (1:length(rate_range))){
  for (j in (1:length(Total_missing_range))){
    temp<-test_function(data = data,rate=rate_range[i],Total_missing = Total_missing_range[j],pram = 12,nRep=c(6,6,6,6,6,6))
    named_vec<-c(temp[,1],temp[,2],temp[,3],temp[,4],temp[,5],temp[,6])
    result_110554<-cbind(result_110554,named_vec)
  }
}

write.csv(result_110554,'result_110554.csv')
