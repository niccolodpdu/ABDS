...
data_FP<-data[data[,3]=='FP',]
data_FC<-data[data[,3]=='FC',]
data_sorted<-rbind(data_NL,data_FS,data_FP,data_FC)
...
test_function<-function(data,rate,Total_missing,SG_threshold=0.98,iCEG_threshold=0.95,missing_rate_threshold=0.6,
  ...
  SGlist<-cotMG(Sest=before_missing_super_sample,cos.thres = 0.5,per = 5)  
  ...
  for(i in 1:length(nRep)){
    data_SG_high <- c(data_SG_high,data_grnd_full_SG[start_x:(start_x + length_SG[i] - 1), 
                                                         start_y : (start_y + nRep[i] - 1)])
    ABDS_imputed_SG_high <- c(ABDS_imputed_SG_high,ABDS_imputed_SG[start_x:(start_x + length_SG[i] - 1), 
                                                                       start_y : (start_y + nRep[i] - 1)])
    PPCA_imputed_SG_high <- c(PPCA_imputed_SG_high,PPCA_imputed_SG[start_x:(start_x + length_SG[i] - 1), 
                                                                       start_y : (start_y + nRep[i] - 1)])
    Half_min_imputed_SG_high <- c(Half_min_imputed_SG_high,Half_min_imputed_SG[start_x:(start_x + length_SG[i] - 1), 
                                                                                   start_y : (start_y + nRep[i] - 1)])
    Mean_imputed_SG_high <- c(Mean_imputed_SG_high,Mean_imputed_SG[start_x:(start_x + length_SG[i] - 1), 
                                                                       start_y : (start_y + nRep[i] - 1)])
    swKNN_imputed_SG_high <- c(swKNN_imputed_SG_high,swKNN_imputed_SG[start_x:(start_x + length_SG[i] - 1), 
                                                                          start_y : (start_y + nRep[i] - 1)])
    NIPALS_imputed_SG_high <- c(NIPALS_imputed_SG_high,NIPALS_imputed_SG[start_x:(start_x + length_SG[i] - 1), 
                                                                             start_y : (start_y + nRep[i] - 1)])
    SVD_imputed_SG_high <- c(SVD_imputed_SG_high,SVD_imputed_SG[start_x:(start_x + length_SG[i] - 1), 
                                                                    start_y : (start_y + nRep[i] - 1)])
    SVT_imputed_SG_high <- c(SVT_imputed_SG_high,SVT_imputed_SG[start_x:(start_x + length_SG[i] - 1), 
                                                                    start_y : (start_y + nRep[i] - 1)])
...
test1<-test_function(data = data_input ,rate = 0.4,Total_missing = 60000,pram = 10,sd_scaler=3)


result<-NULL
rate_range<-c(0.2,0.4,0.6)
Total_missing_range<-c(round(291*710*0.2),round(291*710*0.3),round(291*710*0.4))
for (i in (1:length(rate_range))){
  for (j in (1:length(Total_missing_range))){
    temp<-test_function(data = data_input,rate=rate_range[i],Total_missing = Total_missing_range[j],pram = 12)
